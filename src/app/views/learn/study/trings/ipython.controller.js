 ...
 */
function IPythonController($scope, $sce, $state, $uibModal, $uibModalStack, $timeout, RestFul, ipCookie) {
    if ($scope.task.extra_data.ipython_url) {
        ...
        if (diff_second > 5) {
            $timeout(function() {
                $scope.ipythonUrl = $sce.trustAsResourceUrl($scope.task.extra_data.ipython_url);
            }, 1000)
        } else {
            $timeout(function() {
                $scope.ipythonUrl = $sce.trustAsResourceUrl($scope.task.extra_data.ipython_url);
...
angular
    .module('appLooker')
    .controller('IPythonController', IPythonController)
