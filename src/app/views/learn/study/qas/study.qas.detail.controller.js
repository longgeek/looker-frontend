/**
 * Learn the Study qa detail controllers
 *
 * Functions (controllers)
 *  - StudyQasDetailController
 */


/**
 * StudyQasDetailController - qa detail
 * used in qas/detail.html view
 */
function StudyQasDetailController($scope, $timeout, $uibModal, $uibModalInstance, RestFul, uuid) {
            ...
            if (val.ctype === "studyqa:replied" && $scope.qa && val.params.studyqa.uuid === $scope.qa.uuid) {
                ...
                for (i in val.notification_uuids) {
                    ...
                    if (!exist) {
                        ...
                        $timeout(function() { $scope.modalScrollToBottom(); }, 0)
                    }
                }
                // 消费该问答的所有回复消息
    ...
    $scope.qa_detail = function(uuid) {
        ...
        RestFul.global(
            {
                "action": "WorkspaceCenter:GetStudyQADetail",
                "params": {"studyqa": uuid}
            },
            function(response) {
                if (!response) { return; };
...
angular
    .module('appLooker')
    .controller('StudyQasDetailController', StudyQasDetailController)
