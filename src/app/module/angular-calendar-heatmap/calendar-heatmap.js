...
moment.defineLocale('zh-cn', {
        ...
        scope.drawYearOverview = function () {
          ...
          items.selectAll('.item-circle')
            ...
            .on('mouseover', function (d) {
              ...
              // Construct tooltip
              var tooltip_html = '';
              tooltip_html += '<div class="header"><strong>用时 ' + (d.total ? scope.formatTime(d.total) : 'No time') + '</strong></div>';
              tooltip_html += '<div>' + moment(d.date).format('YYYY-MM-DD') + '</div><br>';
        ...
        scope.drawMonthOverview = function () {
          ...
          item_block.selectAll('.item-block-rect')
            ...
            .on('mouseover', function(d) {
              ...
              tooltip_html += '<div><strong>用时 ' + (d.value ? scope.formatTime(d.value) : 'No time') + '</strong></div>';
        ...
        scope.drawWeekOverview = function () {
          ...
          item_block.selectAll('.item-block-rect')
            ...
            .on('mouseover', function(d) {
              ...
              var tooltip_html = '';
              tooltip_html += '<div class="header"><strong>' + d.name + '</strong></div><br>';
              tooltip_html += '<div><strong>用时 ' + (d.value ? scope.formatTime(d.value) : 'No time') + '<br>共花费：' + d.value * 0.0028 + '</strong></div>';
              tooltip_html += '<div>' + moment(date).format('YYYY-MM-DD') + '</div>';
        ...
        scope.drawDayOverview = function () {
          ...
          items.selectAll('.item-block')
            ...
            .on('mouseover', function(d) {
              ...
              tooltip_html += '<div class="header"><strong>' + d.name + '</strong><div><br>';
              tooltip_html += '<div><strong>用时 ' + (d.value ? scope.formatTime(d.value) : 'No time') + '<br>共花费：' + d.value * 0.0028 + '</strong></div>';
              tooltip_html += '<div>' + moment(d.date).format('YYYY-MM-DD HH:mm:ss') + '</div>';
        ...
        scope.formatTime = function (seconds) {
          ...
          var time = '';
          if ( hours > 0 ) {
            time += hours === 1 ? '1 小时 ' : hours + ' 小时 ';
          }
          if ( minutes > 0 ) {
            time += minutes === 1 ? '1 分钟' : minutes + ' 分钟';
          }
